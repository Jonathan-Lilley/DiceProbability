 ...
 * 
 * Notes:
 * God this was a pain. srand cannot be run within the rand function, otherwise itll just be the same forever;
 * run it in the main function
 * Im... really bad at array indexing >.> I really need to work on that, its just a pain in the butt.
 * Good commenting tho! keep that up!
 * 
 */
...
// Drops rolls
// God, this was so much more tedious than i expected
// Returns an int pointer to retain a list of roll ints
int* dropRolls(int num, int *rolls, int droptype, int drops, bool verbose){
	...
	// Basically just lists what was dropped or kept
	if(verbose){
		cout << "Keeping: ";
		for(int i=0;i<num-drops;i++){
